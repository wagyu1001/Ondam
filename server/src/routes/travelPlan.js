...
router.post('/generate', async (req, res) => {
  ...
  try {
    ...
    // 개발 환경에서 샘플 데이터 반환
    if (process.env.NODE_ENV === 'development' || !process.env.GEMINI_API_KEY || !process.env.OPEN_API_KEY) {
      console.log(`[${new Date().toISOString()}] 개발 환경 감지 - 샘플 익산 여행 계획 반환`);
      
      const samplePlan = {
        title: "익산 1박 2일 여행",
        summary: "익산의 역사와 문화를 체험하는 알찬 여행 계획",
        duration: "1박 2일",
        days: [
          {
            day: 1,
            ...
            activities: [
              {
                time: "09:00",
                location: "익산 미륵사지",
                description: "백제 최대 규모의 사찰터로 석탑과 유물을 관람",
                type: "관광지",
                coordinates: [35.9908, 126.9756]
              },
              {
                time: "11:30",
                location: "익산 왕궁리 유적",
                description: "백제 무왕의 왕궁터로 추정되는 유적지 탐방",
                type: "관광지",
                coordinates: [35.9486, 126.9575]
              },
              {
                time: "14:00",
                location: "익산 맛집거리",
                description: "익산 특산물인 쌀과 한우를 활용한 맛집에서 점심",
                type: "음식점",
                coordinates: [35.9483, 126.9575]
              },
              {
                time: "16:30",
                location: "익산 보석박물관",
                description: "다양한 보석과 광물을 관람할 수 있는 박물관",
                type: "체험",
                coordinates: [35.9439, 126.9575]
              }
            ]
          },
          {
            ...
            activities: [
              {
                time: "08:30",
                location: "익산 금마저수지",
                description: "아름다운 자연경관을 감상하며 산책",
                type: "관광지",
                coordinates: [35.9686, 126.9575]
              },
              {
                time: "11:00",
                location: "익산 한옥마을",
                description: "전통 한옥 체험과 전통문화 프로그램 참여",
                type: "체험",
                coordinates: [35.9486, 126.9575]
              },
              {
        ...
        recommendations: {
          food: ["익산 한우구이", "금마 쌀국수", "익산 막걸리"],
          attractions: ["미륵사지 석탑", "왕궁리 유적", "보석박물관"],
          tips: ["대중교통 이용 시 KTX 익산역 활용", "미륵사지는 오전 방문 추천", "한옥마을 체험 프로그램 사전 예약 필요"]
        }
      };

      const endTime = Date.now();
      console.log(`[${new Date().toISOString()}] 샘플 여행 계획 생성 완료 - 총 소요시간: ${endTime - startTime}ms`);

      return res.json({
        success: true,
        data: samplePlan,
        message: '개발 환경용 샘플 익산 여행 계획이 생성되었습니다.',
        processingTime: endTime - startTime
      });
    }

    if (!process.env.GEMINI_API_KEY || !process.env.OPEN_API_KEY) {
      console.log(`[${new Date().toISOString()}] API 키 누락 - GEMINI_API_KEY: ${process.env.GEMINI_API_KEY}, OPEN_API_KEY: ${process.env.OPEN_API_KEY}`);
...
router.get('/search', async (req, res) => {
  try {
    ...
    console.log(`[${new Date().toISOString()}] 장소 검색 시작 - 검색어: "${query}"`);
    
    // 샘플 데이터 반환
    const allSamplePlaces = [
      {
        id: 'sample_1',
        title: '전주 한옥마을',
      ...
      },
      {
        id: 'sample_4',
        title: '경기전',
        location: '전주시 완산구 태조로 44',
        coordinates: [35.8142, 127.1530],
        description: '조선 태조 이성계의 어진을 모신 곳입니다.',
        type: '역사유적',
        rating: 4.4,
        price_level: null,
        photos: [],
      ...
      },
      {
        id: 'sample_5',
        title: '전주향교',
        location: '전주시 완산구 향교길 139',
        coordinates: [35.8089, 127.1456],
        description: '조선시대 교육기관으로 사용된 향교입니다.',
        type: '역사유적',
        rating: 4.1,
        price_level: null,
        photos: [],
      ...
      },
      {
        id: 'sample_6',
        title: '전주 남부시장',
        location: '전주시 완산구 풍남문3길 2-3',
        coordinates: [35.8123, 127.1489],
        description: '전통 시장에서 다양한 먹거리를 즐길 수 있습니다.',
        type: '시장',
        rating: 4.0,
        price_level: 1,
        photos: [],
    ...
    // 검색어와 관련된 결과 필터링
    const searchResults = allSamplePlaces.filter(place => 
    ...
    console.log(`[${new Date().toISOString()}] 검색 완료 - 결과: ${searchResults.length}개`);
    ...
    console.error(`[${new Date().toISOString()}] 검색 오류:`, error.message);
